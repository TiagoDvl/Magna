...
> **Já existe — `EmptyComponent`**, criado junto da correção do item 4.3.1 do mapa, que era o mesmo defeito com outra cara: a tela de comissão decidia entre carregar e mostrar com `if (lista.vazia)`, então comissão sem votação girava para sempre. Título obrigatório e descrição opcional. O bloco 11 revisa a aparência dele como revisa o resto, mas o componente não precisa ser inventado.
